...
let TeamController = class TeamController {
    ...
    async create(createTeamDto) {
        return new team_entity_1.TeamEntity(await this.teamService.create(createTeamDto));
    }
    async findAll() {
        const teams = await this.teamService.findAll();
        return teams.map((team) => new team_entity_1.TeamEntity(team));
    }
    async findOne(id) {
        ...
        return new team_entity_1.TeamEntity(team);
    }
    async update(id, updateTeamDto) {
        return new team_entity_1.TeamEntity(await this.teamService.update(id, updateTeamDto));
    }
    async remove(id) {
        return new team_entity_1.TeamEntity(await this.teamService.remove(id));
    }
};
